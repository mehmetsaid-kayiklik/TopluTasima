...
object S {
    ...
    // ── Drive Core ──
    fun navVehicles(l: AppLanguage) = m(l, "Araçlar", "Fahrzeuge", "Vehicles")
    fun driveVehiclesTitle(l: AppLanguage) = m(l, "Araçlar", "Fahrzeuge", "Vehicles")
    fun driveVehiclesSubtitle(l: AppLanguage) = m(l, "Araçlarını ve manuel sürüşlerini yönet", "Fahrzeuge und manuelle Fahrten verwalten", "Manage vehicles and manual drives")
    fun driveAddVehicle(l: AppLanguage) = m(l, "Araç ekle", "Fahrzeug hinzufügen", "Add vehicle")
    fun driveNoVehiclesTitle(l: AppLanguage) = m(l, "Henüz araç yok", "Noch keine Fahrzeuge", "No vehicles yet")
    fun driveNoVehiclesBody(l: AppLanguage) = m(l, "İnternet olmasa da ilk aracını ekleyebilirsin.", "Du kannst dein erstes Fahrzeug auch offline hinzufügen.", "You can add your first vehicle even while offline.")
    fun driveRetry(l: AppLanguage) = m(l, "Tekrar dene", "Erneut versuchen", "Try again")
    fun driveBack(l: AppLanguage) = m(l, "Geri", "Zurück", "Back")
    fun driveVehicleDetails(l: AppLanguage) = m(l, "Araç detayı", "Fahrzeugdetails", "Vehicle details")
    fun driveNewVehicle(l: AppLanguage) = m(l, "Yeni araç", "Neues Fahrzeug", "New vehicle")
    fun driveEditVehicle(l: AppLanguage) = m(l, "Aracı düzenle", "Fahrzeug bearbeiten", "Edit vehicle")
    fun driveBasicInfo(l: AppLanguage) = m(l, "Temel bilgiler", "Grunddaten", "Basic information")
    fun driveDisplayName(l: AppLanguage) = m(l, "Araç adı", "Fahrzeugname", "Vehicle name")
    fun driveBrand(l: AppLanguage) = m(l, "Marka", "Marke", "Brand")
    fun driveModel(l: AppLanguage) = m(l, "Model", "Modell", "Model")
    fun driveModelYear(l: AppLanguage) = m(l, "Model yılı", "Baujahr", "Model year")
    fun driveLicensePlate(l: AppLanguage) = m(l, "Plaka", "Kennzeichen", "License plate")
    fun driveFuelType(l: AppLanguage) = m(l, "Yakıt türü", "Kraftstoffart", "Fuel type")
    fun driveInitialOdometer(l: AppLanguage) = m(l, "Başlangıç kilometresi", "Anfangskilometerstand", "Initial odometer")
    fun driveManualCurrentOdometer(l: AppLanguage) = m(l, "Güncel manuel kilometre", "Aktueller manueller Kilometerstand", "Current manual odometer")
    fun driveAssignedPerson(l: AppLanguage) = m(l, "Atanmış kişi", "Zugeordnete Person", "Assigned person")
    fun drivePersonUnassigned(l: AppLanguage) = m(l, "Kişi atanmamış", "Keine Person zugeordnet", "No person assigned")
    fun drivePersonUnavailable(l: AppLanguage) = m(l, "Kişi bilgisi kullanılamıyor", "Personendaten nicht verfügbar", "Person details unavailable")
    fun driveRemoveAssignment(l: AppLanguage) = m(l, "Atamayı kaldır", "Zuordnung entfernen", "Remove assignment")
    fun driveNotes(l: AppLanguage) = m(l, "Not", "Notiz", "Notes")
    fun driveProfileGeneral(l: AppLanguage) = m(l, "Genel profil", "Allgemeines Profil", "General profile")
    fun driveProfileTechnical(l: AppLanguage) = m(l, "Teknik", "Technik", "Technical")
    fun driveProfilePurchase(l: AppLanguage) = m(l, "Satın alma", "Kauf", "Purchase")
    fun driveCountryCode(l: AppLanguage) = m(l, "Ülke kodu", "Ländercode", "Country code")
    fun driveTransmission(l: AppLanguage) = m(l, "Şanzıman", "Getriebe", "Transmission")
    fun driveBodyType(l: AppLanguage) = m(l, "Kasa tipi", "Karosserie", "Body type")
    fun driveColor(l: AppLanguage) = m(l, "Renk", "Farbe", "Color")
    fun driveTrimLevel(l: AppLanguage) = m(l, "Donanım paketi", "Ausstattung", "Trim level")
    fun driveVin(l: AppLanguage) = m(l, "VIN", "FIN", "VIN")
    fun driveEngineCode(l: AppLanguage) = m(l, "Motor kodu", "Motorcode", "Engine code")
    fun driveEngineDisplacement(l: AppLanguage) = m(l, "Motor hacmi", "Hubraum", "Engine displacement")
    fun driveEnginePower(l: AppLanguage) = m(l, "Motor gücü", "Motorleistung", "Engine power")
    fun driveTireSize(l: AppLanguage) = m(l, "Lastik ölçüsü", "Reifengröße", "Tire size")
    fun driveRegistrationDate(l: AppLanguage) = m(l, "Tescil tarihi", "Zulassungsdatum", "Registration date")
    fun drivePurchaseDate(l: AppLanguage) = m(l, "Satın alma tarihi", "Kaufdatum", "Purchase date")
    fun drivePurchasePrice(l: AppLanguage) = m(l, "Satın alma fiyatı", "Kaufpreis", "Purchase price")
    fun driveCurrencyCode(l: AppLanguage) = m(l, "Para birimi (ISO 4217)", "Währung (ISO 4217)", "Currency (ISO 4217)")
    fun driveInspectionDueDate(l: AppLanguage) = m(l, "Muayene bitiş tarihi", "HU-Fälligkeit", "Inspection due date")
    fun driveInsuranceDueDate(l: AppLanguage) = m(l, "Sigorta bitiş tarihi", "Versicherungsfälligkeit", "Insurance due date")
    fun drivePhotos(l: AppLanguage) = m(l, "Fotoğraflar", "Fotos", "Photos")
    fun driveAddPhoto(l: AppLanguage) = m(l, "Fotoğraf ekle", "Foto hinzufügen", "Add photo")
    fun driveTakePhoto(l: AppLanguage) = m(l, "Fotoğraf çek", "Foto aufnehmen", "Take photo")
    fun driveNoPhotos(l: AppLanguage) = m(l, "Henüz fotoğraf yok.", "Noch keine Fotos.", "No photos yet.")
    fun driveMakePrimary(l: AppLanguage) = m(l, "Ana fotoğraf yap", "Als Hauptfoto", "Make primary")
    fun drivePrimaryPhoto(l: AppLanguage) = m(l, "Ana fotoğraf", "Hauptfoto", "Primary photo")
    fun driveDeletePhoto(l: AppLanguage) = m(l, "Fotoğrafı sil", "Foto löschen", "Delete photo")
    fun drivePhotoPending(l: AppLanguage) = m(l, "Yükleme bekliyor", "Upload ausstehend", "Upload pending")
    fun drivePhotoFailed(l: AppLanguage) = m(l, "Fotoğraf eşitlenemedi", "Foto konnte nicht synchronisiert werden", "Photo could not sync")
    fun drivePhotoLoading(l: AppLanguage) = m(l, "Fotoğraf hazırlanıyor", "Foto wird vorbereitet", "Preparing photo")
    fun driveTotalDistance(l: AppLanguage) = m(l, "Toplam sürüş mesafesi", "Gesamtfahrstrecke", "Total drive distance")
    fun driveTripCount(l: AppLanguage) = m(l, "Sürüş sayısı", "Anzahl Fahrten", "Drive count")
    fun driveTripCountValue(count: Int, l: AppLanguage) = m(l, "$count sürüş", "$count Fahrten", "$count drives")
    fun driveLastUsed(l: AppLanguage) = m(l, "Son kullanım", "Letzte Nutzung", "Last used")
    fun driveNoUsage(l: AppLanguage) = m(l, "Henüz kullanılmadı", "Noch nicht genutzt", "Not used yet")
    fun driveCurrentOdometer(l: AppLanguage) = m(l, "Güncel kilometre", "Aktueller Kilometerstand", "Current odometer")
    fun driveOdometerManual(l: AppLanguage) = m(l, "Manuel değer", "Manueller Wert", "Manual value")
    fun driveOdometerEstimated(l: AppLanguage) = m(l, "Sürüşlerden tahmini", "Aus Fahrten geschätzt", "Estimated from drives")
    fun driveOdometerUnavailable(l: AppLanguage) = m(l, "Kilometre bilgisi yok", "Kein Kilometerstand", "Odometer unavailable")
    fun driveOdometerInconsistent(l: AppLanguage) = m(l, "Manuel kilometre, sürüşlerden hesaplanan tahmini değerden düşük. Veriler değiştirilmedi.", "Der manuelle Kilometerstand liegt unter der Schätzung aus Fahrten. Die Daten wurden nicht verändert.", "The manual odometer is below the estimate from drives. No data was changed.")
    fun driveRecentTrips(l: AppLanguage) = m(l, "Son sürüşler", "Letzte Fahrten", "Recent drives")
    fun driveNoTrips(l: AppLanguage) = m(l, "Bu araç için henüz sürüş yok.", "Für dieses Fahrzeug gibt es noch keine Fahrten.", "There are no drives for this vehicle yet.")
    fun driveAddTrip(l: AppLanguage) = m(l, "Sürüş ekle", "Fahrt hinzufügen", "Add drive")
    fun driveEditTrip(l: AppLanguage) = m(l, "Sürüşü düzenle", "Fahrt bearbeiten", "Edit drive")
    fun driveManualTrip(l: AppLanguage) = m(l, "Manuel sürüş", "Manuelle Fahrt", "Manual drive")
    fun driveVehicle(l: AppLanguage) = m(l, "Araç", "Fahrzeug", "Vehicle")
    fun driveStartDate(l: AppLanguage) = m(l, "Başlangıç tarihi", "Startdatum", "Start date")
    fun driveStartTime(l: AppLanguage) = m(l, "Başlangıç saati", "Startzeit", "Start time")
    fun driveEndDate(l: AppLanguage) = m(l, "Bitiş tarihi", "Enddatum", "End date")
    fun driveEndTime(l: AppLanguage) = m(l, "Bitiş saati", "Endzeit", "End time")
    fun driveStartOdometer(l: AppLanguage) = m(l, "Başlangıç kilometresi", "Kilometerstand am Start", "Start odometer")
    fun driveEndOdometer(l: AppLanguage) = m(l, "Bitiş kilometresi", "Kilometerstand am Ende", "End odometer")
    fun driveDistance(l: AppLanguage) = m(l, "Mesafe", "Strecke", "Distance")
    fun drivePurpose(l: AppLanguage) = m(l, "Amaç", "Zweck", "Purpose")
    fun driveStartLocation(l: AppLanguage) = m(l, "Başlangıç yeri adı", "Startort", "Start location name")
    fun driveEndLocation(l: AppLanguage) = m(l, "Bitiş yeri adı", "Zielort", "End location name")
    fun driveCalculatedDistance(value: String, l: AppLanguage) = m(l, "Kilometre farkından hesaplanan mesafe: $value", "Aus Kilometerständen berechnete Strecke: $value", "Distance calculated from odometers: $value")
    fun driveOptional(l: AppLanguage) = m(l, "İsteğe bağlı", "Optional", "Optional")
    fun driveDateHint(l: AppLanguage) = m(l, "GG.AA.YYYY", "TT.MM.JJJJ", "DD.MM.YYYY")
    fun driveTimeHint(l: AppLanguage) = m(l, "SS:DD", "HH:MM", "HH:MM")
    fun driveDeleteVehicleTitle(l: AppLanguage) = m(l, "Araç silinsin mi?", "Fahrzeug löschen?", "Delete vehicle?")
    fun driveDeleteVehicleBody(l: AppLanguage) = m(l, "Araç cihazdan hemen kaldırılır ve silme işlemi çevrimdışı kuyruğa alınır.", "Das Fahrzeug wird sofort vom Gerät entfernt und die Löschung offline vorgemerkt.", "The vehicle is removed from this device immediately and its deletion is queued offline.")
    fun driveDeleteVehicleCascadeBody(count: Int, l: AppLanguage) = m(l, "Bu araca bağlı $count sürüş var. Araç ve bağlı sürüşlerin tümü silinecek.", "Mit diesem Fahrzeug sind $count Fahrten verknüpft. Fahrzeug und alle verknüpften Fahrten werden gelöscht.", "This vehicle has $count linked drives. The vehicle and every linked drive will be deleted.")
    fun driveDeleteVehicleAndTrips(l: AppLanguage) = m(l, "Aracı ve sürüşleri sil", "Fahrzeug und Fahrten löschen", "Delete vehicle and drives")
    fun driveDeleteTripTitle(l: AppLanguage) = m(l, "Sürüş silinsin mi?", "Fahrt löschen?", "Delete drive?")
    fun driveDeleteTripBody(l: AppLanguage) = m(l, "Sürüş cihazdan hemen kaldırılır ve silme işlemi çevrimdışı kuyruğa alınır.", "Die Fahrt wird sofort vom Gerät entfernt und die Löschung offline vorgemerkt.", "The drive is removed from this device immediately and its deletion is queued offline.")
    fun driveSyncPending(l: AppLanguage) = m(l, "Eşitleme bekliyor", "Synchronisierung ausstehend", "Waiting to sync")
    fun driveSyncing(l: AppLanguage) = m(l, "Eşitleniyor", "Wird synchronisiert", "Syncing")
    fun driveSynced(l: AppLanguage) = m(l, "Eşitlendi", "Synchronisiert", "Synced")
    fun driveSyncRetry(l: AppLanguage) = m(l, "Eşitleme daha sonra yeniden denenecek", "Synchronisierung wird später erneut versucht", "Sync will be retried later")
    fun driveSyncFailed(l: AppLanguage) = m(l, "Eşitleme müdahale gerektiriyor", "Synchronisierung erfordert Aufmerksamkeit", "Sync needs attention")
    fun driveSyncUnknown(l: AppLanguage) = m(l, "Eşitleme durumu bilinmiyor", "Synchronisierungsstatus unbekannt", "Sync status unknown")
    fun driveFieldRequired(l: AppLanguage) = m(l, "Bu alan zorunludur.", "Dieses Feld ist erforderlich.", "This field is required.")
    fun driveInvalidNumber(l: AppLanguage) = m(l, "Geçerli bir ondalık sayı girin (ör. 12,5 veya 12.5).", "Gib eine gültige Dezimalzahl ein (z. B. 12,5 oder 12.5).", "Enter a valid decimal number (for example 12.5).")
    fun driveInvalidDate(l: AppLanguage) = m(l, "Tarihi GG.AA.YYYY biçiminde girin.", "Gib das Datum im Format TT.MM.JJJJ ein.", "Enter the date as DD.MM.YYYY.")
    fun driveInvalidTime(l: AppLanguage) = m(l, "Saati SS:DD biçiminde girin.", "Gib die Zeit im Format HH:MM ein.", "Enter the time as HH:MM.")
    fun driveInvalidCode(l: AppLanguage) = m(l, "Geçerli bir ISO kodu girin.", "Gib einen gültigen ISO-Code ein.", "Enter a valid ISO code.")
    fun driveDisplayNameRequired(l: AppLanguage) = m(l, "Araç adı boş bırakılamaz.", "Der Fahrzeugname darf nicht leer sein.", "Vehicle name cannot be empty.")
    fun driveModelYearInvalid(l: AppLanguage) = m(l, "Geçerli bir model yılı girin.", "Gib ein gültiges Baujahr ein.", "Enter a valid model year.")
    fun driveNegativeOdometer(l: AppLanguage) = m(l, "Kilometre negatif olamaz.", "Der Kilometerstand darf nicht negativ sein.", "Odometer cannot be negative.")
    fun driveCurrentBeforeInitial(l: AppLanguage) = m(l, "Güncel kilometre başlangıç kilometresinden küçük olamaz.", "Der aktuelle Kilometerstand darf nicht unter dem Anfangswert liegen.", "Current odometer cannot be below the initial odometer.")
    fun driveVehicleRequired(l: AppLanguage) = m(l, "Bir araç seçin.", "Wähle ein Fahrzeug aus.", "Select a vehicle.")
    fun driveVehicleNotFound(l: AppLanguage) = m(l, "Araç bu hesapta bulunamadı.", "Das Fahrzeug wurde in diesem Konto nicht gefunden.", "The vehicle was not found in this account.")
    fun driveEndBeforeStart(l: AppLanguage) = m(l, "Bitiş başlangıçtan önce olamaz.", "Das Ende darf nicht vor dem Start liegen.", "End cannot be before start.")
    fun driveNegativeDistance(l: AppLanguage) = m(l, "Mesafe negatif olamaz.", "Die Strecke darf nicht negativ sein.", "Distance cannot be negative.")
    fun driveEndOdometerBeforeStart(l: AppLanguage) = m(l, "Bitiş kilometresi başlangıç kilometresinden küçük olamaz.", "Der Endkilometerstand darf nicht unter dem Startwert liegen.", "End odometer cannot be below start odometer.")
    fun driveDistanceRequired(l: AppLanguage) = m(l, "Mesafe veya başlangıç ve bitiş kilometrelerini girin.", "Gib die Strecke oder Start- und Endkilometerstand ein.", "Enter a distance or both start and end odometers.")
    fun driveDistanceMismatch(l: AppLanguage) = m(l, "Mesafe, başlangıç ve bitiş kilometreleri arasındaki farkla uyuşmuyor.", "Die Strecke stimmt nicht mit der Differenz der Kilometerstände überein.", "Distance does not match the difference between start and end odometers.")
    fun driveAuthRequired(l: AppLanguage) = m(l, "Devam etmek için yeniden giriş yapın.", "Melde dich erneut an, um fortzufahren.", "Sign in again to continue.")
    fun driveOwnershipMismatch(l: AppLanguage) = m(l, "Bu kayıt mevcut hesaba ait değil.", "Dieser Eintrag gehört nicht zum aktuellen Konto.", "This record does not belong to the current account.")
    fun driveRecordNotFound(l: AppLanguage) = m(l, "Kayıt bulunamadı.", "Eintrag nicht gefunden.", "Record not found.")
    fun driveRecordDeleted(l: AppLanguage) = m(l, "Bu kayıt zaten silinmiş.", "Dieser Eintrag wurde bereits gelöscht.", "This record has already been deleted.")
    fun driveDatabaseFailure(l: AppLanguage) = m(l, "Değişiklik yerel veritabanına kaydedilemedi.", "Die Änderung konnte nicht lokal gespeichert werden.", "The change could not be saved locally.")
    fun driveSchedulingFailure(l: AppLanguage) = m(l, "Değişiklik cihazda kaydedildi; eşitleme daha sonra yeniden planlanacak.", "Die Änderung wurde lokal gespeichert; die Synchronisierung wird später erneut geplant.", "The change was saved on this device; sync will be scheduled again later.")
    fun driveUnknownFailure(l: AppLanguage) = m(l, "İşlem tamamlanamadı. Tekrar deneyin.", "Der Vorgang konnte nicht abgeschlossen werden. Versuche es erneut.", "The action could not be completed. Try again.")
    fun driveVehicleSaved(l: AppLanguage) = m(l, "Araç kaydedildi.", "Fahrzeug gespeichert.", "Vehicle saved.")
    fun driveTripSaved(l: AppLanguage) = m(l, "Sürüş kaydedildi.", "Fahrt gespeichert.", "Drive saved.")
    fun driveVehicleDeleted(l: AppLanguage) = m(l, "Araç silme kuyruğuna alındı.", "Fahrzeuglöschung wurde vorgemerkt.", "Vehicle deletion was queued.")
    fun driveTripDeleted(l: AppLanguage) = m(l, "Sürüş silme kuyruğuna alındı.", "Fahrtlöschung wurde vorgemerkt.", "Drive deletion was queued.")
    fun driveDismiss(l: AppLanguage) = m(l, "Kapat", "Schließen", "Dismiss")
    fun driveSearch(l: AppLanguage) = m(l, "Araç ara", "Fahrzeug suchen", "Search vehicles")
    fun driveFilter(l: AppLanguage) = m(l, "Filtre", "Filter", "Filter")
    fun driveAllFuelTypes(l: AppLanguage) = m(l, "Tüm yakıt türleri", "Alle Kraftstoffarten", "All fuel types")
    fun driveAllAssignments(l: AppLanguage) = m(l, "Tüm atamalar", "Alle Zuordnungen", "All assignments")
    fun driveAssignedOnly(l: AppLanguage) = m(l, "Kişi atanmış", "Person zugeordnet", "Assigned")
    fun driveUnassignedOnly(l: AppLanguage) = m(l, "Kişi atanmamış", "Nicht zugeordnet", "Unassigned")
    fun driveSort(l: AppLanguage) = m(l, "Sırala", "Sortieren", "Sort")
    fun driveSortName(l: AppLanguage) = m(l, "Ada göre", "Nach Name", "By name")
    fun driveSortPlate(l: AppLanguage) = m(l, "Plakaya göre", "Nach Kennzeichen", "By plate")
    fun driveSortFuel(l: AppLanguage) = m(l, "Yakıt türüne göre", "Nach Kraftstoff", "By fuel type")
    fun driveSortAssignment(l: AppLanguage) = m(l, "Atamaya göre", "Nach Zuordnung", "By assignment")
    fun driveSortLastUsed(l: AppLanguage) = m(l, "Son kullanıma göre", "Nach letzter Nutzung", "By last used")
    fun driveSortDistance(l: AppLanguage) = m(l, "Toplam kilometreye göre", "Nach Gesamtstrecke", "By total distance")
    fun driveAscending(l: AppLanguage) = m(l, "Artan", "Aufsteigend", "Ascending")
    fun driveDescending(l: AppLanguage) = m(l, "Azalan", "Absteigend", "Descending")
    fun driveSelect(l: AppLanguage) = m(l, "Seç", "Auswählen", "Select")
    fun driveFinishSelection(l: AppLanguage) = m(l, "Seçimi bitir", "Auswahl beenden", "Finish selection")
    fun driveSelectedCount(count: Int, l: AppLanguage) = m(l, "$count araç seçildi", "$count Fahrzeuge ausgewählt", "$count vehicles selected")
    fun driveDeleteSelected(l: AppLanguage) = m(l, "Seçilenleri sil", "Auswahl löschen", "Delete selected")
    fun driveBulkDeleteTitle(l: AppLanguage) = m(l, "Seçilen araçlar silinsin mi?", "Ausgewählte Fahrzeuge löschen?", "Delete selected vehicles?")
    fun driveBulkDeleteBody(count: Int, l: AppLanguage) = m(l, "$count araç ve bağlı sürüşleri çevrimdışı silme kuyruğuna alınacak.", "$count Fahrzeuge und verknüpfte Fahrten werden zur Offline-Löschung vorgemerkt.", "$count vehicles and their linked drives will be queued for offline deletion.")
    fun driveBulkDeleted(l: AppLanguage) = m(l, "Seçilen araçlar silme kuyruğuna alındı.", "Ausgewählte Fahrzeuge wurden zur Löschung vorgemerkt.", "Selected vehicles were queued for deletion.")
    fun driveNoFilterResults(l: AppLanguage) = m(l, "Bu arama ve filtrelerle eşleşen araç yok.", "Keine Fahrzeuge entsprechen dieser Suche und den Filtern.", "No vehicles match this search and these filters.")
    fun driveHealth(l: AppLanguage) = m(l, "Veri sağlığı", "Datenqualität", "Data health")
    fun driveHealthIssueCount(count: Int, l: AppLanguage) = m(l, "$count veri uyarısı", "$count Datenhinweise", "$count data health issues")
    fun driveProvenance(l: AppLanguage) = m(l, "Alan kaynakları", "Feldquellen", "Field sources")
    fun driveProvenanceLocal(l: AppLanguage) = m(l, "Yerel", "Lokal", "Local")
    fun driveProvenanceRemote(l: AppLanguage) = m(l, "Bulut", "Cloud", "Remote")
    fun driveProvenanceMerged(l: AppLanguage) = m(l, "Birleştirilmiş", "Zusammengeführt", "Merged")
    fun driveProvenanceUnknown(l: AppLanguage) = m(l, "Bilinmiyor", "Unbekannt", "Unknown")
    fun driveLatestSync(l: AppLanguage) = m(l, "Son Drive eşitlemesi", "Letzte Drive-Synchronisierung", "Latest Drive sync")
}
